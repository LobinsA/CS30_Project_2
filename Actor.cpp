...
void Protester::doSomething() // UNDER CONSTRUCTION
{
	...
	if (accessToWorld()->CPUspotsUser(this)) {
		// If the Hardcore Protester is in the action of following DiggerMan,
		// empty out its path of coordinates (start fresh), then yell at the DiggerMan
		if (hasCellPhoneTracker() && !path.empty()) {
			while (!path.empty())
				path.pop();
		}
	...
	{
		// if HC Protester has a path to the DiggerMan, move one step along that path
		// note: "the path" is the ideal/shortest path to the DiggerMan
		if (!path.empty()) {
			pathNode p = path.front();
			setDirection(p.m_dir);
			moveTo(p.m_x, p.m_y);
			...
			doSomething_aux(); // check if DiggerMan will trigger the cellphone signal
			if (!path.empty()) // if the signal was triggered, then HC Protester has a path of coordinates to the DiggerMan, return
				return;
		}
	}
